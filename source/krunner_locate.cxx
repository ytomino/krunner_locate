...
static bool lt(queried_item_t const &left, queried_item_t const &right)
{
	...
	std::size_t l_path_count = count_units(left.path, 0, l_sep);
	std::size_t r_path_count = count_units(right.path, 0, r_sep);
	if(l_path_count != r_path_count){
		return l_path_count < r_path_count;
	}
...
void LocateRunner::match(KRunner::RunnerContext &context)
{
	...
	for(
		...
		if(sep >= 0){
			...
			QString path = iter->path.left(sep);
			if(path.startsWith(home_path)){
				path.replace(0, home_path.size(), QStringLiteral("~/"));
			}
